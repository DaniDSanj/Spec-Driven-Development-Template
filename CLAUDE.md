...
- **La carga útil de andamiaje ya en su ruta final de destino** (`.claude/`, `.specify/memory/`, `docs/`, `.githooks/`, `.github/workflows/ci.yml`, `.gitignore`, `.gitattributes`, `LICENSE`): esto es lo que "Use this template" entrega intacto a cada proyecto nuevo, sin ningún paso de copia.
...
- `SECURITY.md` — qué ejecuta la plantilla en la máquina de quien la usa, los límites conocidos de los guards y cómo reportar un fallo de seguridad en privado.
...
| `.githooks/pre-push` | Red local opcional (`git config core.hooksPath .githooks`) con las mismas comprobaciones que el job `quality` del CI |
...
| `.github/workflows/ci.yml` | Workflow de CI (ruff/ty/pytest), estático y gateado por la existencia de `pyproject.toml` y `src/` |
| `.gitignore` | Escrito para este stack: secretos, Python, entornos `uv` (pero **no** `uv.lock`), estado local de Obsidian y `settings.local.json` |
