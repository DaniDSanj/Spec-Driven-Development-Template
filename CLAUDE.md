...
- **Ficheros de la plantilla que se conservan y se adaptan** en el proyecto downstream: `SECURITY.md` (lo que el repo ejecuta, sus capas de seguridad y qué hacer si se filtra un secreto; remiten a él `.githooks/pre-commit` y `pre_edit_guard_sensitive.sh`) y `LICENSE`.
