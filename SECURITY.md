...
Cuando el CI encuentra algo, el secreto **ya está en GitHub**: trátalo como filtrado y rótalo.

Sus límites:

- **Es opcional y local**: solo actúa si has activado `core.hooksPath` y tienes `gitleaks` instalado.
  Sin `gitleaks`, avisa y deja pasar el commit. `git commit --no-verify` lo salta.
...
- **Acciones fijadas por SHA completo**, no por tag: un tag se puede mover a otro commit (es como se
  ...
  versión.
- **`.github/dependabot.yml`** abre cada mes, si hay novedades, una PR por ecosistema hacia `dev` con las actualizaciones de las
  acciones (nuevos SHA) y de las dependencias `uv`. Cada una pasa `quality` y requiere revisión
  humana: Dependabot propone, no mergea.
...
`.env`, claves y certificados, y backups/dumps de base de datos, y el subagente `security-reviewer`
revisa las superficies sensibles de cada feature antes del cierre — pero la seguridad del proyecto final es responsabilidad de quien lo escribe.
